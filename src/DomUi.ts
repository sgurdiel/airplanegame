...
export default class DomUi {
  ...
  constructor() {
    ...
    if (gamingElement == null)
      throw new Error("Fatal: Missing HTML Element with id 'game'");
    this.gameContainer = gamingElement;
    const reloadingElement = document.getElementById('reloadingMsg');
    if (reloadingElement == null)
      throw new Error("Fatal: Missing HTML Element with id 'reloadingMsg'");
    this.reloadingMsg = reloadingElement;
    const scoreElement = document.getElementById('score');
    if (scoreElement == null)
      throw new Error("Fatal: Missing HTML Element with id 'score'");
    this.scoreElement = scoreElement;
    const gameendContainer = document.getElementById('gameend');
    if (gameendContainer == null)
      throw new Error("Fatal: Missing HTML Element with id 'gameend'");
    ...
    if (magazineContainer == null)
      throw new Error("Fatal: Missing HTML Element with id 'magazineMsg'");
    this.magazineContainer = magazineContainer;
    const infoOverlay = document.getElementById('infoOverlay');
    if (infoOverlay == null)
      throw new Error("Fatal: Missing HTML Element with id 'infoOverlay'");
    this.infoOverlay = infoOverlay;
    const errorContainer = document.getElementById('error');
    if (errorContainer == null)
      throw new Error("Fatal: Missing HTML Element with id 'error'");
    this.errorContainer = errorContainer;
    const radarScreen = document.getElementById('radarImg');
    if (radarScreen === null)
      throw new Error("Fatal: Missing HTML Element with id 'radarImg'");
    this.radarScreen = radarScreen;
    const radarMsg = document.getElementById('radarMsg');
    if (radarMsg === null)
      throw new Error("Fatal: Missing HTML Element with id 'radarMsg'");
  ...
  public supportedScreenSize(): boolean {
    ...
    this.verticalPositionOffset =
      this.getGameContainer().getBoundingClientRect().top;
    this.maxLeftPosGaimingContainer =
      this.getGameContainer().getBoundingClientRect().left +
      this.gameContainerWith;
    this.maxVerticalPosition =
      this.verticalPositionOffset + this.gameContainerHeight;
    const supportedScreenSize =
      this.screenHeight >= 768 && this.screenWidth >= 1024;
    this.displayError(
      !supportedScreenSize,
      'Minimum screen resolution not meet (w:1024px h:768px',
    );
    return supportedScreenSize;
  }

  public gameElementWithinGameContainerBoundaries(
  ...
  public fireAirplaneFireRoundEvent(
    listener: EventListenerOrEventListenerObject,
  ): void {
  ...
  public randomTopPos(element: GameElement): number {
    return Math.round(
      this.verticalPositionOffset +
        Math.random() * (this.gameContainerHeight - element.getImgH()),
    );
  }
