...
export default class Airplane {
    ...
    private fireRoundsCapacity: number = 10
    private fireRoundsAvailable: FireRound[] = []
    private fireRoundsFired: FireRound[] = []
    ...
    constructor(private ui: DomUi) {
        ...
        this.element.move({topPos: Math.round(this.ui.getScreenHeight() / 2), leftPos: (this.ui.getMaxLeftPosGaimingContainer() - this.element.getImgL() - 4)})
    ...
    public move(): void {
        let position: GameElementPosition = {
            ...
            leftPos: this.ui.getMaxLeftPosGaimingContainer() - this.element.getImgL() - 4
        }
        position = this.ui.gameElementWithinGameContainerBoundaries(position, this.element)
        this.element.move(position)
    }

    public fireRound(): void {
        let fireRound: FireRound | undefined = this.fireRoundsAvailable.pop()
        if (fireRound !== undefined){
            this.ui.appendGameElement(fireRound.getElement())
            const position: GameElementPosition = {topPos: this.element.getPosition().topPos + Math.round(this.element.getImgH()/2), leftPos: this.element.getPosition().leftPos}
            fireRound.fire(position)
            this.fireRoundsFired.push(fireRound)
            this.ui.displayMagazineMsg(this.fireRoundsAvailable.length)
        }
        if (this.fireRoundsAvailable.length === 0 && this.reloading === false ) {
            this.ui.displayReloadingMsg(this.reloadTime)
            this.reloading = true
            asyncDelay(this.reloadTime).then(() => {
                this.reload()
            })
        }
    }
    ...
    public moveFiredRounds(): void {
        for (let index = 0; index < this.getFireRoundsFired().length; index++) {
            ...
            let newLeftPos: number = position.leftPos - this.fireRoundSpeed
            if (newLeftPos >= 0) {
                this.fireRoundsFired[index].move({topPos: position.topPos, leftPos: newLeftPos})
            } else {
