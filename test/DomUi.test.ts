...
describe('DomUi', () => {
  ...
  test('Unsupported screen resolution', () => {
    ...
    new DomUi();
    expect(
      window.document.getElementById('errorContainer')?.style.visibility ===
        'visible',
    );
    expect(
      window.document.getElementById('errorContainer')?.innerHTML ===
        '<p>Minimum screen resolution not meet (w:1024px h:768px</p>',
    );
  ...
  test('Propeties are defined and default values set', () => {
    ...
    expect(ui.getMaxLeftPosGaimingContainer()).toBe(
      ui.getGameContainer().getBoundingClientRect().left +
        ui.getGameContainerWith(),
    );
  ...
  test.each([
    ...
    jest.spyOn(ge, 'getImgH').mockImplementation(() => {
      return 50;
    });
    expect(
      ui.gameElementWithinGameContainerBoundaries({ topPos: t, leftPos: 0 }, ge)
        .topPos,
    ).toBe(e);
  });
  ...
  test('Append GameElement', () => {
    ...
    jest.spyOn(ge, 'getHtmlElement').mockImplementation(() => {
      return div;
    });
  ...
  test('Display game end', () => {
    ...
    const ui = new DomUi();
    ui.displayGameEnd();
    expect(window.document.getElementById('gameend')?.style.visibility).toBe(
      'visible',
    );
  });
  ...
  test('Display magazine message', () => {
    ...
    expect(window.document.getElementById('reloadingMsg')?.style.display).toBe(
      'none',
    );
    expect(window.document.getElementById('magazineMsg')?.style.display).toBe(
      'block',
    );
    expect(window.document.getElementById('magazineMsg')?.innerHTML).toBe(
      'Fire rounds: 5',
    );
  });
  ...
  test('Await display reloading', async () => {
    ...
    await new Promise((done) => {
      ...
      expect(
        window.document.getElementById('reloadingMsg')?.style.display,
      ).toBe('none');
      expect(window.document.getElementById('magazineMsg')?.style.display).toBe(
        'block',
      );
  ...
  test('Radar message', () => {
    ...
    expect(window.document.getElementById('radarImg')?.childNodes.length).toBe(
      1,
    );
    expect(
      window.document.getElementById('radarImg')?.firstChild,
    ).toBeInstanceOf(HTMLImageElement);
    const image = window.document.getElementById('radarImg')
  ...
  test('DOM event subscriber', () => {
    ...
    const ui = new DomUi();
    let eventOutput = '';
    ui.windowResizeEvent(() => {
      eventOutput = 'windowResize';
    });
    ui.fireAirplaneFireRoundEvent(() => {
      eventOutput = 'documentMousedown';
    });
    ui.moveAirplaneEvent(() => {
      eventOutput = 'divMousemove';
    });
