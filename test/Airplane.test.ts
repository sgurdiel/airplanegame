...
beforeEach(() => {
  ...
  jest
    .spyOn(GameElement.prototype, 'getPosition')
    .mockReturnValue({ topPos: 400, leftPos: 604 });
  jest
    .spyOn(FireRound.prototype, 'getElement')
    .mockReturnValue(new GameElement(ui, 'div', 58, 92, ''));
});
...
describe('Airplane', () => {
  test('Propeties are defined and default values set', () => {
    ...
    expect(gameElementMoveMock).toHaveBeenCalledWith({
      topPos: 400,
      leftPos: 800 - a.getElement().getImgL() - 4,
    });
  });

  test('Move', () => {
    const a = new Airplane(ui);
    const gameElementWithinGameContainerBoundariesMock = jest.spyOn(
      ui,
      'gameElementWithinGameContainerBoundaries',
    );
    a.move();
    expect(
      gameElementWithinGameContainerBoundariesMock.mock.calls[0][0],
    ).toMatchObject({
      topPos: 450 - Math.round(a.getElement().getImgH() / 2),
      leftPos: 800 - a.getElement().getImgL() - 4,
    });
  });
  ...
  test('Fire round', () => {
    ...
    const a = new Airplane(ui);
    const fireRoundsFired = a.getFireRoundsFired().length;
    const fireRoundsAvailable = a.getFireRoundsAvailable().length;
    a.fireRound();
    ...
    expect(fireRoundFireMethodMock.mock.calls[0][0]).toMatchObject({
      topPos: 429,
      leftPos: 604,
    });
  });
  ...
  test('Fire round moves', () => {
    ...
    const fireRoundGetPositionMock = jest
      .spyOn(FireRound.prototype, 'getPosition')
    ...
    expect(fireRoundMoveMethodMock.mock.calls[0][0]).toMatchObject({
      topPos: 429,
      leftPos: 604 - a.getFireRoundSpeed(),
    });
  });

  test('Fire round gets removed when screen limit reached', () => {
    const position = { topPos: 429, leftPos: 0 };
    jest.spyOn(FireRound.prototype, 'getPosition').mockReturnValue(position);
    const a = new Airplane(ui);
    a.fireRound();
    a.moveFiredRounds();
  ...
  test('Firing last fire round will reload', async () => {
    const asyncDelayFunctionMock = jest
      .spyOn(Helpers, 'asyncDelay')
      .mockImplementation(() => {
