...
describe('Radar', () => {
  ...
  test('Check MissileAtomic destruction', async () => {
    ...
    jest
      .spyOn(fr, 'getPosition')
      .mockReturnValue({ topPos: 400, leftPos: 400 });
  ...
  test('When towers destroyed only the tower destroy message should be present in the queue', () => {
    ...
    for (let index = 0; index < 4; index++) {
      if (index < 3) {
        ...
        r.scan();
        expect(r.getMsgQueue().length).toBe(1);
        expect(r.getMsgQueue()[0]).toMatchObject({
          displayTime: 0,
          msg: '<span style="color:lightred">TOWERS DESTROYED RETURN TO BASE</span>',
          image: '/img/M17491.jpg',
        });
      }
    }
  });
